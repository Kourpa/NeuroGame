...
public class Player extends GameObject {
    ...
    //System.out.println(directionVector);
        ...
        //System.out.println("Hit: (" + getX()+", "+getY()+") velY = "+velY);
                ...
                if(getY() < Library.VERTICAL_MIN){
                    lastVelocityY = 0.002;
                }
                else if(getY() + getHeight() > Library.VERTICAL_MAX){
                    lastVelocityY = -0.002;
                }
...
//  }
    ...
    public void loseHealth(double hitX, double hitY, int damage){
        if(collisionTimer > 0 || invulnerable){
            return;
        }
    ...
    public void render(Graphics2D g){
        ...
        if(health == 2){
            g.drawImage(Library.getSprites().get("pDmg1"), xx, yy,
                        null);
        }

        else if(health == 1){
            g.drawImage(Library.getSprites().get("pDmg2"), xx,
                        yy, null);
        }
