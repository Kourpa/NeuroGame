...
public class GameController
{
  ...
  private void playUpdate(long deltaTime)
  {
    ...
    // Player input.
    keyHandler();
    // If the world is scrolling, update the positions.
    if (scrolling)
    {
      double newDeltaX = world.update(graphics, deltaTime);
      ...
      player.setLocation(newPlayerX, player.getY());
      deltaX = newDeltaX;
      Library.setDeltas(deltaX, 0);
    }
    ...
    updateObjectList(objects, deltaTime);
   ...
   */
  public void updateObjectList(List<GameObject> list, long deltaTime)
  {
//    for (ListIterator<GameObject> li = list.listIterator(); li.hasNext();)
//    {
//      GameObject o = li.next();
//
//      // Update object.
//      o.update(deltaTime);
//      // If the object changed its visibility state, log it.
//      boolean isVisible = o.isVisible();
//      boolean isOnScreen = Library.isOnScreen(o);
//
//      // System.out.println("GameController.objectHelper(): "+o.getName() +
//      // ", isVisible="+isVisible+", isOnScreen = " + isOnScreen);
//
...
//        {
//          o.setIsVisible(true);
//          log(o.getName() + " (ID: " + o.getId() + ")" + " became visible.");
//        }
//        else
//        {
//          o.setIsVisible(false);
//          log(o.getName() + " (ID: " + o.getId() + ")" + " moved off-screen.");
//        }
//      }
//      // If the object moved too far off screen to the left, remove it.
//      double x = o.getX();
//      double y = o.getY();
//      double worldWidth = world.getWidth();
//      double worldHeight = world.getHeight();
//      double worldXMin = deltaX - worldWidth;
//      double worldXMax = deltaX + worldWidth + worldWidth;
//      double worldYMin = deltaY - worldHeight;
//      double worldYMax = deltaY + worldHeight + worldHeight;
//      if (x < worldXMin || x > worldXMax || y < worldYMin || y > worldYMax)
//      {
//        li.remove();
//      }
//      else
//      {
//        // If still active and on-screen after update, draw.
//        
//        o.render(graphics);
//        if (!o.isActive())
//        {
...
//    }
  ...
  private void keyHandler()
  {
    ...
    if (joystick != null)
    {
      if (joystick.isButtonPressed(0)) usePowerUp();
      else if (joystick.isButtonPressed(1)) usePowerUp();
      else if (joystick.isButtonPressed(2)) usePowerUp();
      else if (joystick.isButtonPressed(3)) usePowerUp();
      else if (joystick.isButtonPressed(4)) usePowerUp();
      else if (joystick.isButtonPressed(5)) usePowerUp();
    }
    ...
    if (globalDebug)
    {
      ...
      if (inputs.get("enemy_a") && debug)
      {
        ...
        objects.add(new EnemyStraight(deltaX + 1.0, deltaY + 0.5, world));
      }
      if (inputs.get("enemy_b") && debug)
      {
        inputs.put("enemy_b", false);
        objects.add(new EnemySinusoidal(deltaX + 1.0, deltaY + 0.5, world));
      }
      if (inputs.get("enemy_c") && debug)
      {
        inputs.put("enemy_c", false);
        objects.add(new EnemyFollow(deltaX + 1.0, deltaY + 0.5, world));
      }
      // Debug coin spawner.
      if (inputs.get("coin") && debug)
      {
        inputs.put("coin", false);
        objects.add(new Coin(deltaX + 1.0, deltaY + 0.5, world));
      }
      // Debug zapper spawner.
      if (inputs.get("zapper") && debug)
      {
        inputs.put("zapper", false);
        objects.add(new Zapper(deltaX + 1.25, deltaY + 0.3, deltaX + 1.25,
            deltaY + 0.7, world));
      }
    }
  }
